...
let consumableNotificationsLoading = false;
...
function initializeInteractiveWidgets(scope = document) {
    ...
    initializeConsumableNotifications(scope);
    syncApartmentTemplateSelectState();
...
function initializeConsumableNotifications(scope = document) {
    const roots = [];
    if (scope instanceof Element && scope.matches('[data-consumable-notifications]')) {
        roots.push(scope);
    }

    if (scope.querySelectorAll) {
        roots.push(...scope.querySelectorAll('[data-consumable-notifications]'));
    }

    roots.forEach((root) => {
        if (!(root instanceof HTMLElement) || root.dataset.consumableNotificationsReady === 'true') {
            return;
        }

        root.dataset.consumableNotificationsReady = 'true';
        root.addEventListener('click', handleConsumableNotificationClick);
        root.addEventListener('submit', handleConsumableNotificationSubmit);
        loadConsumableNotifications(root);
    });

    if (document.documentElement.dataset.consumableNotificationOutsideReady !== 'true') {
        document.documentElement.dataset.consumableNotificationOutsideReady = 'true';
        document.addEventListener('click', (event) => {
            const target = event.target instanceof Element ? event.target : null;
            if (target?.closest('[data-consumable-notifications]')) {
                return;
            }
...
function handleConsumableNotificationClick(event) {
    const trigger = event.target instanceof Element ? event.target.closest('[data-consumable-notifications-trigger]') : null;
    if (!(trigger instanceof HTMLElement)) {
        return;
    }

    event.preventDefault();
    const root = trigger.closest('[data-consumable-notifications]');
    if (!(root instanceof HTMLElement)) {
        return;
    }
    ...
    updateConsumableNotificationOpenState();
...
async function handleConsumableNotificationSubmit(event) {
    const form = event.target instanceof Element ? event.target.closest('[data-consumable-restock-form]') : null;
    if (!(form instanceof HTMLFormElement)) {
        return;
    }

    event.preventDefault();
    const root = form.closest('[data-consumable-notifications]');
    if (!(root instanceof HTMLElement)) {
        return;
    }
    ...
    try {
        const response = await fetch(form.action, {
            ...
            headers: {
                'X-Requested-With': 'XMLHttpRequest',
                'Accept': 'application/json',
                ...(getAppCsrfToken() ? {'X-App-Csrf': getAppCsrfToken()} : {}),
            },
        });

        const payload = await response.json();
        if (!response.ok || !payload?.success) {
            throw new Error(payload?.message || 'Réapprovisionnement impossible.');
        }

        renderConsumableNotifications(root, payload);
        showToast(payload.message || 'Stock mis à jour.');
    } catch (error) {
        showToast(error.message || 'Réapprovisionnement impossible.', 'error');
    } finally {
        if (submitButton instanceof HTMLButtonElement) {
            submitButton.disabled = false;
        }
...
async function loadConsumableNotifications(root) {
    if (!(root instanceof HTMLElement) || consumableNotificationsLoading) {
        return;
    }

    const endpoint = root.dataset.consumableNotificationsEndpoint || '';
    if (!endpoint) {
        return;
    }

    consumableNotificationsLoading = true;
    try {
        ...
        consumableNotificationsLoading = false;
    }
}

function renderConsumableNotifications(root, payload) {
    const panel = root.querySelector('[data-consumable-notifications-panel]');
    const badge = root.querySelector('[data-consumable-notifications-badge]');
    const count = Number.parseInt(payload?.count ?? 0, 10) || 0;

    if (panel instanceof HTMLElement && typeof payload?.html === 'string') {
...
function closeConsumableNotificationPanels() {
    document.querySelectorAll('[data-consumable-notifications]').forEach((root) => {
        const panel = root.querySelector('[data-consumable-notifications-panel]');
...
function updateConsumableNotificationOpenState() {
    const hasOpenPanel = Array.from(document.querySelectorAll('[data-consumable-notifications-panel]'))
        .some((panel) => panel instanceof HTMLElement && !panel.hidden);

    document.body.classList.toggle('consumable-notifications-open', hasOpenPanel);
}

function isConsumableNotificationInteractionActive() {
    const activeElement = document.activeElement;
    return activeElement instanceof Element && Boolean(activeElement.closest('[data-consumable-notifications]'));
}
...
function openArrivalReservationPickerModal() {
    const modalElement = document.getElementById('arrivalReservationApartmentModal');
    if (!(modalElement instanceof HTMLElement)) {
        return;
    }
...
function selectArrivalReservationApartment(trigger) {
    const reservationModal = document.getElementById('apartmentReservationModal');
    if (!(reservationModal instanceof HTMLElement)) {
        return;
    }

    const form = reservationModal.querySelector('form');
    if (form instanceof HTMLFormElement) {
        const action = trigger.getAttribute('data-reservation-action');
        const ranges = trigger.getAttribute('data-reservation-ranges') || '[]';

        if (action) {
            form.action = action;
        }

        form.setAttribute('data-reservation-ranges', ranges);
    }

    const selectedName = reservationModal.querySelector('[data-arrival-reservation-selected-name]');
    if (selectedName instanceof HTMLElement) {
        selectedName.textContent = trigger.getAttribute('data-reservation-apartment-name') || 'Appartement sélectionné';
    }

    hideModal('arrivalReservationApartmentModal');
...
function showAirbnbEquipmentModal(modalElement) {
    if (!(modalElement instanceof HTMLElement)) {
        return;
    }

    moveAirbnbEquipmentModalToBody(modalElement);
    modalElement.scrollTop = 0;
    showModalElement(modalElement);
}

function moveAirbnbEquipmentModalToBody(modalElement) {
    if (!(modalElement instanceof HTMLElement) || modalElement.parentElement === document.body) {
        return;
    }

    if (modalElement.id) {
        document.querySelectorAll('.airbnb-equipment-modal').forEach((duplicate) => {
            if (duplicate instanceof HTMLElement && duplicate !== modalElement && duplicate.parentElement === document.body) {
                duplicate.remove();
            }
        });
    }
    ...
    placeholder.dataset.airbnbModalPlaceholder = modalElement.id || 'airbnb-equipment-modal';
    modalElement.parentNode?.insertBefore(placeholder, modalElement);
    modalElement.dataset.airbnbMovedToBody = 'true';
    modalElement.dataset.airbnbPlaceholderId = placeholder.dataset.airbnbModalPlaceholder;
    document.body.appendChild(modalElement);
}

function restoreMovedAirbnbEquipmentModal(modalElement) {
    if (!(modalElement instanceof HTMLElement) || modalElement.dataset.airbnbMovedToBody !== 'true') {
        return;
    }

    const placeholderId = modalElement.dataset.airbnbPlaceholderId || '';
    const placeholder = Array.from(document.querySelectorAll('[data-airbnb-modal-placeholder]'))
        .find((candidate) => candidate instanceof HTMLElement && candidate.dataset.airbnbModalPlaceholder === placeholderId);

    delete modalElement.dataset.airbnbMovedToBody;
    delete modalElement.dataset.airbnbPlaceholderId;

    if (placeholder instanceof HTMLElement && placeholder.parentNode) {
        placeholder.parentNode.insertBefore(modalElement, placeholder);
        placeholder.remove();
        return;
    }
...
document.addEventListener('click', (event) => {
    ...
    if (airbnbEquipmentModalTrigger instanceof HTMLElement) {
        ...
        const roomContent = airbnbEquipmentModalTrigger.closest('#airbnb-room-content');
        const modalElement = modalSelector
            ? (roomContent?.querySelector(modalSelector) || document.querySelector(modalSelector))
            : null;
        if (modalElement instanceof HTMLElement) {
            showAirbnbEquipmentModal(modalElement);
        }
        return;
    }
    ...
    const arrivalReservationPickerTrigger = event.target instanceof Element ? event.target.closest('[data-arrival-reservation-picker-trigger]') : null;
    if (arrivalReservationPickerTrigger instanceof HTMLElement) {
        event.preventDefault();
        openArrivalReservationPickerModal();
        return;
    }

    const arrivalReservationApartmentChoice = event.target instanceof Element ? event.target.closest('[data-arrival-reservation-apartment-choice]') : null;
    if (arrivalReservationApartmentChoice instanceof HTMLElement) {
        event.preventDefault();
        selectArrivalReservationApartment(arrivalReservationApartmentChoice);
        return;
    }
...
function syncTopBarOnScroll() {
    ...
    if (shouldCompactNav && !isConsumableNotificationInteractionActive()) {
        closeConsumableNotificationPanels();
    }
...
function forceHideModalElement(modalElement) {
    ...
    restoreMovedAirbnbEquipmentModal(modalElement);
}
